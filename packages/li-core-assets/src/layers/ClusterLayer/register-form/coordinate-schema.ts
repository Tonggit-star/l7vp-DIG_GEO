...
/**
 * 聚合图层的坐标 schema：聚合在 L7 中基于 coordinates=[lng,lat]（supercluster），语义上只适合
 * 经纬度(x/y)点数据，故只提供 点度(table)/度分秒(dms)，去掉 geometry（fromValues 仍保留 geometry
 * 兜底分支以防御历史数据）。
 */
              ...
              'x-reactions': [
                {
                  ...
                  fulfill: {
                    state: {
                      visible: '{{ $deps[0] === "table" || $deps[0] === "dms" }}',
                      title: '{{ $deps[0] === "dms" ? "经度(度分秒)" : "经度" }}',
                    },
                  },
                },
              ],
